...
<head>
  ...
  <title>لوحة الإدارة | Clinic Evaluator</title>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-login:hover { background: #134e4a; }
    ...
    .logo { font-size: 1.3rem; font-weight: 800; color: #134e4a; }
    ...
    #last-updated { color: #6b7280; font-size: 0.85rem; }
    ...
    .stat-value { font-size: 1.8rem; font-weight: 800; color: #0f766e; }
    .stat-label { color: #6b7280; font-size: 0.9rem; }

    .settings-section, .table-section {
      background: #ffffff;
      border-radius: 16px;
      padding: 20px;
      margin-bottom: 24px;
    ...
    .settings-section h2 { margin-bottom: 16px; font-size: 1.1rem; color: #134e4a; font-weight: 700; }
    .settings-grid {
      display: grid;
    ...
    .setting-title { font-weight: 700; color: #1f2937; }
    .toggle-switch {
      position: relative;
      width: 48px;
      height: 24px;
    ...
    .toggle-switch.active { background: #0f766e; }
    .toggle-switch::after {
      content: '';
      position: absolute;
      width: 20px;
    ...
    .setting-status { font-size: 0.9rem; margin-bottom: 12px; }
    .users-list { margin-top: 12px; max-height: 200px; overflow-y: auto; }
    .user-item {
      display: flex;
      justify-content: space-between;
      align-items: center;
    ...
    .user-info { color: #6b7280; font-size: 0.8rem; }
    .btn-small {
      padding: 4px 10px;
      border: none;
      border-radius: 6px;
    ...
    .btn-add:hover { background: #134e4a; }
    ...
    .table-header h3 { font-size: 1.1rem; color: #134e4a; font-weight: 700; }
    ...
    .badge-warning { background: #fefce8; color: #92400e; }
    .btn-details {
      padding: 6px 14px;
      background: #0f766e;
      color: white;
    ...
    .btn-details:hover { background: #134e4a; }
    ...
    .modal-header h3 { font-size: 1.1rem; color: #134e4a; font-weight: 700; }
    .btn-close {
      background: none;
      border: none;
      color: #6b7280;
    ...
    .detail-label { color: #6b7280; font-size: 0.8rem; margin-bottom: 4px; }
    ...
    .answer-question { font-size: 0.9rem; margin-bottom: 6px; color: #1f2937; }
    .answer-selected { color: #0f766e; font-weight: 700; }
    .scores-grid {
      display: grid;
    ...
    .score-name { font-size: 0.85rem; color: #6b7280; margin-bottom: 8px; }
    .score-value { font-size: 1.5rem; font-weight: 800; color: #0f766e; }
    ...
    .form-group input:focus { outline: none; border-color: #14b8a6; }
...
<body>
  <div id="login-screen" class="login-screen">
    <div class="login-box">
      <h2>لوحة الإدارة</h2>
      <p>Clinic Evaluator Dashboard</p>
      <form id="login-form">
        <input type="password" id="login-password" placeholder="أدخل كلمة المرور" required autofocus>
        <div id="login-error" class="login-error hidden">كلمة المرور غير صحيحة</div>
    ...
    <div class="admin-container">
      <header class="admin-header">
        ...
        <div class="header-actions">
          <button id="btn-refresh" class="btn-icon" title="تحديث">🔄</button>
          <button id="btn-logout" class="btn-icon" title="خروج">🚪</button>
      ...
      <section class="stats-grid">
        ...
        <div class="stat-card">
          <div class="stat-icon">🏥</div>
          <div class="stat-info">
            <div class="stat-value" id="stat-clinics">0</div>
            <div class="stat-label">عدد العيادات</div>
          </div>
        </div>
      </section>
      ...
      <section class="settings-section">
        ...
        <div class="settings-grid" id="assessment-settings"></div>
      </section>

      <section class="filters-section">
        <div class="search-box">
          <input type="text" id="search-input" placeholder="بحث: اسم، بريد، هاتف، عيادة...">
          <button id="btn-search">🔍</button>
        </div>
        <div class="filter-group">
          <select id="filter-type">
            <option value="">كل التقييمات</option>
            <option value="patient-journey">رحلة المريض</option>
            <option value="clinic-performance">أداء العيادة</option>
          </select>
          ...
          <select id="filter-sort">
            ...
            <option value="score-high">الدرجة: الأعلى</option>
            <option value="score-low">الدرجة: الأقل</option>
      ...
      <section class="table-section">
        <div class="table-header">
          <h3>📋 التقييمات</h3>
          <span id="results-count">0 نتيجة</span>
        </div>
  ...
  <div class="modal-overlay hidden" id="user-modal">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="user-form">
          <input type="hidden" id="user-assessment-key">
          ...
          <div class="form-group">
            ...
            <input type="number" id="user-max-uses" value="1" min="1" required>
          </div>
          <div class="form-group">
            <label>مدة الصلاحية (أيام)</label>
            <input type="number" id="user-expiry-days" value="30" min="1" required>
          </div>
          <div class="form-actions">
            <button type="submit" class="btn-primary">إنشاء</button>
            <button type="button" class="btn-secondary" id="btn-cancel-user">إلغاء</button>
          </div>
        </form>
      </div>
    </div>
  ...
  <script>
    const EDGE_URL = 'https://oaqpzaarppccbnepffxx.supabase.co/functions/v1/admin-auth';
    const ADMIN_PASSWORD_HASH = '6051fc84a7a0d74c225fb18a496b09952da5642e60723ecae543298edd7d82d6';

    async function edgeRequest(action, data = {}, filter = {}, table = '') {
      const res = await fetch(EDGE_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action, data, filter, table, password: sessionStorage.getItem('admin_pwd_hash') || '' })
      });
    ...
    class AdminDashboard {
      ...
      checkLogin() {
        ...
        try { return JSON.parse(auth).authenticated === true; } catch { return false; }
      }
      ...
      setupLoginEvents() {
        document.getElementById('login-form').addEventListener('submit', async (e) => {
          ...
          if (hash === ADMIN_PASSWORD_HASH) {
            sessionStorage.setItem('admin_pwd_hash', hash);
      ...
      logout() {
        ...
        sessionStorage.removeItem('admin_pwd_hash');
        location.reload();
      }
      ...
      async loadConfig() {
        ...
        } catch (e) { console.warn('[Admin] Config load failed:', e); }
      }
      ...
      async loadAllData() {
        ...
        try {
          const [leadsRes, sessionsRes, answersRes, scoresRes, settingsRes, usersRes] = await Promise.all([
            edgeRequest('select', { columns: '*', order: { column: 'created_at', direction: 'desc' }, limit: 1000 }, {}, 'leads'),
            edgeRequest('select', { columns: '*', limit: 1000 }, {}, 'sessions'),
            edgeRequest('select', { columns: '*', limit: 10000 }, {}, 'answers'),
            edgeRequest('select', { columns: '*', limit: 10000 }, {}, 'scores'),
            edgeRequest('select', { columns: '*' }, {}, 'assessment_settings'),
            edgeRequest('select', { columns: '*' }, {}, 'assessment_users')
          ]);
          this.leads = leadsRes || [];
          ...
          return `
            <div class="setting-card">
              ...
              <div class="setting-status">
                ${isEnabled ? '<span style="color:#0f766e;font-weight:700">🔒 محمي</span>' : '<span style="color:#6b7280">🔓 مفتوح</span>'}
              </div>
              ${isEnabled ? `
                <div class="users-list">
                  ${aUsers.length === 0 ? '<div style="color:#6b7280;font-size:0.85rem;text-align:center;padding:12px;">لا يوجد مستخدمين</div>' : aUsers.map(u => `
                    <div class="user-item">
                      <div>
                        <div>${u.username}</div>
      ...
      async toggleAuth(assessmentKey) {
        ...
        try {
          if (setting) await edgeRequest('update', { auth_enabled: newState }, { assessment_key: assessmentKey }, 'assessment_settings');
          else await edgeRequest('insert', { assessment_key: assessmentKey, auth_enabled: newState }, {}, 'assessment_settings');
          const res = await edgeRequest('select', { columns: '*' }, {}, 'assessment_settings');
          this.settings = res || [];
          this.renderSettings();
          this.showToast(newState ? 'تم تفعيل الحماية' : 'تم إلغاء الحماية', 'success');
        } catch (err) { this.showToast('فشل تغيير الحماية', 'error'); }
      }
      ...
      hideAddUserModal() { document.getElementById('user-modal').classList.add('hidden'); }
      ...
      async addUser(e) {
        ...
        try {
          await edgeRequest('insert', { assessment_key: aKey, username, password_hash: hash, max_uses: maxUses, used_count: 0, expires_at: expiresAt.toISOString(), active: true }, {}, 'assessment_users');
          this.users = (await edgeRequest('select', { columns: '*' }, {}, 'assessment_users')) || [];
          this.renderSettings(); this.hideAddUserModal(); this.showToast('تم إنشاء المستخدم', 'success');
        } catch (err) { this.showToast('فشل إنشاء المستخدم', 'error'); }
      }

      async deleteUser(userId) {
        if (!confirm('هل أنت متأكد من حذف هذا المستخدم؟')) return;
        try {
          await edgeRequest('delete', {}, { id: userId }, 'assessment_users');
          this.users = (await edgeRequest('select', { columns: '*' }, {}, 'assessment_users')) || [];
          this.renderSettings(); this.showToast('تم حذف المستخدم', 'success');
        } catch (err) { this.showToast('فشل حذف المستخدم', 'error'); }
      }

      setupDashboardEvents() {
        document.getElementById('btn-logout').addEventListener('click', () => this.logout());
        document.getElementById('search-input').addEventListener('input', () => { clearTimeout(this.debounceTimer); this.debounceTimer = setTimeout(() => this.applyFilters(), 300); });
        ...
        document.getElementById('btn-refresh').addEventListener('click', () => { this.loadAllData().then(() => { this.renderStats(); this.renderSettings(); this.applyFilters(); this.updateLastUpdated(); this.showToast('تم التحديث', 'success'); }); });
      ...
      applyFilters() {
        ...
        if (search) filtered = filtered.filter(l => (l.full_name + l.email + l.phone + l.clinic_name + l.country).toLowerCase().includes(search));
        ...
        this.filteredLeads = filtered; this.currentPage = 1; this.renderTable();
      }
      ...
      renderTable() {
        ...
        if (pageData.length === 0) {
          tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:40px;color:#6b7280;">لا توجد بيانات</td></tr>';
        } else {
      ...
      renderPagination(totalPages) {
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          else if (i === this.currentPage - 2 || i === this.currentPage + 2) html += '<span style="color:#6b7280;padding:8px;">...</span>';
        }
      ...
      async showDetails(leadId) {
        ...
        modalBody.innerHTML = `
          <div class="detail-section"><h4>👤 معلومات المريض</h4><div class="detail-grid">
            ...
            <div class="detail-item"><div class="detail-label">سنوات التشغيل</div><div class="detail-value">${lead.years || '--'}</div></div>
            ...
            <div class="detail-item"><div class="detail-label">التاريخ</div><div class="detail-value">${lead.created_at ? new Date(lead.created_at).toLocaleString('ar-SA') : '--'}</div></div>
            <div class="detail-item"><div class="detail-label">الحالة</div><div class="detail-value">${lead.completed ? '✅ مكتمل' : '⏳ غير مكتمل'}</div></div>
          </div></div>
          ${scores.length > 0 ? `<div class="detail-section"><h4>📊 درجات المحاور</h4><div class="scores-grid">${scores.map(s => `<div class="score-card"><div class="score-name">${s.axis_name_ar || s.axis_id}</div><div class="score-value">${s.percentage != null ? s.percentage.toFixed(1) : '--'}%</div></div>`).join('')}<div style="margin-top:16px;text-align:center;padding:16px;background:#f8fafc;border-radius:8px;border:1px solid #e5e7eb;grid-column:1/-1;"><div style="color:#6b7280;font-size:0.9rem;">الدرجة الكلية</div><div style="font-size:2rem;font-weight:800;color:#0f766e;">${lead.score_percentage != null ? lead.score_percentage.toFixed(1) : '--'}%</div></div></div></div>` : ''}
          ${answers.length > 0 ? `<div class="detail-section"><h4>📝 الإجابات</h4><div class="answers-list">${answers.map((a, i) => `<div class="answer-item"><div class="answer-question">${a.question_text || `سؤال ${i + 1}`}</div><div class="answer-selected">${this.getAnswerLabel(a.question_id, a.answer_value)}</div></div>`).join('')}</div></div>` : '<div style="text-align:center;color:#6b7280;padding:20px;">لا توجد إجابات مسجلة</div>'}
        `;
      ...
      getAnswerLabel(questionId, value) {
        ...
        for (const [key, assessment] of Object.entries(this.config.assessment_types || {})) {
          ...
          if (question) { const option = question.options?.find(o => o.value === value); if (option) return option.label; }
        }
        return `قيمة: ${value}`;
      }
      ...
      showLoading(show) {
        ...
        if (!overlay) { overlay = document.createElement('div'); overlay.id = 'loading-overlay'; overlay.className = 'loading-overlay hidden'; overlay.innerHTML = '<div class="spinner"></div>'; document.body.appendChild(overlay); }
      ...
      showToast(message, type = 'success') {
        ...
        if (!toast) return;
        toast.textContent = message; toast.className = `toast ${type}`; toast.classList.remove('hidden');
